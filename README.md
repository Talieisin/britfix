...
- **Quotations**: explicit emphasised quotations are preserved verbatim, so `*"color"*`, `_“color”_` and the bold forms `**"color"**` and `__"color"__` keep their spelling. Ordinary italic emphasis without quotes still converts, and quotations inside code spans or code blocks are not scanned at all. Set `strategies.markdown.preserve_quoted_prose` to `true` in `config.json` (boolean, default `false`) to preserve every quoted run of prose, not just the emphasised ones; a malformed value is a fatal config error.
